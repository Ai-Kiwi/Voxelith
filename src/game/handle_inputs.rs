...
use crate::{
    game::{InputEvent, pixel::PixelTypes, world::WorldData},
    utils::{Vec3, raycast_test},
};
                ...
                match player_input_update {
                    ...
                    InputEvent::Click(position, facing_dir) => {
                        ...
                        'rayloop: for ray in raycast_test(position, facing_dir.normalize()) {
                            i += 1;
                            if world.get_pixel_data(ray.x as i32, ray.y as i32, ray.z as i32)
                                != Some(PixelTypes::Air)
                            {
                                for x in -5..=5 {
                                    for y in -5..=5 {
                                        for z in -5..=5 {
                                            world.set_pixel_data(
                                                (
                                                    ray.x as i32 + x,
                                                    ray.y as i32 + y,
                                                    ray.z as i32 + z,
