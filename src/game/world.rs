...
impl WorldData {
    ...
    pub fn set_pixel_data(&mut self, pixel_pos: (i32, i32, i32), pixel: PixelTypes) {
        self.pixel_edit_queue
            .push((pixel_pos.0, pixel_pos.1, pixel_pos.2, pixel));
    }

    pub fn test_collision_area(
        &self,
        start_pixel_pos: (i32, i32, i32),
        end_pixel_pos: (i32, i32, i32),
    ) -> bool {
                    ...
                    for local_y in 0..16 as i32 {
                        for local_z in 0..16 as i32 {
                            for local_x in 0..16 as i32 {
                                ...
                                if block_x <= end_chunk_x
                                    && block_x >= start_chunk_x
                                    && block_y <= end_chunk_y
                                    && block_y >= start_chunk_y
                                    && block_z <= end_chunk_z
                                    && block_z >= start_chunk_z
                                {
                                    match chunk_data {
                                        Some(data) => {
                                            if data
                                                .get_relative_pixel(local_x, local_y, local_z)
                                                .is_solid()
                                            {
                                                return true;
                                            }
