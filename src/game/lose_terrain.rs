use std::{any::Any, collections::{HashMap, HashSet}, sync::{Arc, mpsc::Sender}};

use cgmath::num_traits::Euclid;
use rayon::iter::{IntoParallelRefIterator, ParallelIterator};

use crate::{game::{chunk::Chunk, world::{self, WorldData}}, render::update_state::LoseObjectRenderDataUpdate, utils::Vec3};
...
pub struct LoseObject {
    ...
    pub chunks_updated : HashSet<(i32,i32,i32)>
...
impl LoseObject {
    pub fn find_lose_terrain_collision(&mut self, world_data : &WorldData) -> Vec<((i32, i32, i32),(i32, i32, i32))> {
        let rigidbody_chunk_list : Vec<(&(i32,i32,i32),&Arc<Chunk>)> = self.chunks_loc.iter().filter(|chunk| {
            let mut valid = false;
            ...
            valid = (l_chunk_x as f32 * 16.0) - self.velocity.x < ((self.min_max_x.0 as f32) * 16.0) || valid;
            valid = (l_chunk_x as f32 * 16.0) - self.velocity.x > ((self.min_max_x.1 as f32) * 16.0) || valid;

            valid = (l_chunk_y as f32 * 16.0) - self.velocity.y < ((self.min_max_y.0 as f32) * 16.0) || valid;
            valid = (l_chunk_y as f32 * 16.0) - self.velocity.y > ((self.min_max_y.1 as f32) * 16.0) || valid;

            valid = (l_chunk_z as f32 * 16.0) - self.velocity.z < ((self.min_max_z.0 as f32) * 16.0) || valid;
            valid = (l_chunk_z as f32 * 16.0) - self.velocity.z > ((self.min_max_z.1 as f32) * 16.0) || valid;
        ...
        let world_chunk_list : Vec<(&(i32,i32,i32),&Arc<Chunk>)> = touching_world_chunks.iter().filter_map(|chunk_pos| {
        ...
        let world_chunk_references_to_lose_object: HashMap<&(i32,i32,i32),Vec<Option<&Arc<Chunk>>>> = world_chunk_list.iter().map(|w_chunk| {
            ...
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_min, l_chunk_y_min, l_chunk_z_min)).and_then(|index| self.chunks.get(*index)));
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_max, l_chunk_y_min, l_chunk_z_min)).and_then(|index| self.chunks.get(*index)));
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_min, l_chunk_y_max, l_chunk_z_min)).and_then(|index| self.chunks.get(*index)));
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_max, l_chunk_y_max, l_chunk_z_min)).and_then(|index| self.chunks.get(*index)));
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_min, l_chunk_y_min, l_chunk_z_max)).and_then(|index| self.chunks.get(*index)));
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_max, l_chunk_y_min, l_chunk_z_max)).and_then(|index| self.chunks.get(*index)));
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_min, l_chunk_y_max, l_chunk_z_max)).and_then(|index| self.chunks.get(*index)));
            reference_chunks.push(self.chunks_loc.get(&(l_chunk_x_max, l_chunk_y_max, l_chunk_z_max)).and_then(|index| self.chunks.get(*index)));
        ...
        for chunk in world_chunk_list {
            let w_chunk_references_l_chunk = world_chunk_references_to_lose_object.get(&chunk.0).unwrap();
                ...
                for z in 0..16 as i32 {
                    for x in 0..16 as i32 {
                        ...
                        if let Some(chunk) = w_chunk_references_l_chunk.get(offset).unwrap() {
                            ...
                            if chunk.get_relative_pixel(l_chunk_local_voxel_x, l_chunk_local_voxel_y, l_chunk_local_voxel_z).is_solid() == true && self_solid == true {
...
pub fn handle_lose_object_update(lose_objects : &mut LoseTerrain, entity_render_tx: &Sender<LoseObjectRenderDataUpdate>,) {
