use crate::{
    game::pixel::PixelTypes,
    mesh_creation::{
        ChunkMeshCreateRequest,
        create_triangles::{TriangleSide, add_triangle},
        pix_colors::calculate_pix_color,
    },
    utils::{Material, Mesh, Vertex},
};
        ...
        for local_x in (0..16).step_by(lod as usize) {
            for local_y in (0..16).step_by(lod as usize) {
                for local_z in (0..16).step_by(lod as usize) {
                    ...
                    let (color, material) = calculate_pix_color(
                        pixel_value,
                        world_pixel_x as i32,
                        world_pixel_y as i32,
                        world_pixel_z as i32,
                    );

                    if local_x == 0 {
                        if let Some(neighbor) = request.x_negative_chunk_neighbor.as_ref() {
                            if neighbor
                                .get_relative_pixel(16 - step, local_y, local_z)
                                .should_be_face(transparent)
                            {
                                add_triangle(
                                    &mut vertices,
                                    TriangleSide::Left,
                        ...
                        if request_chunk_data
                            .get_relative_pixel(local_x - step, local_y, local_z)
                            .should_be_face(transparent)
                        {
                            add_triangle(
                                &mut vertices,
                    ...
                    if local_x + step > 15 {
                        if let Some(neighbor) = request.x_positive_chunk_neighbor.as_ref() {
                            if neighbor
                                .get_relative_pixel(-1 + step, local_y, local_z)
                                .should_be_face(transparent)
                            {
                                add_triangle(
                                    &mut vertices,
                    ...
                    if local_y == 0 {
                        if let Some(neighbor) = request.y_negative_chunk_neighbor.as_ref() {
                            if neighbor
                                .get_relative_pixel(local_x, 16 - step, local_z)
                                .should_be_face(transparent)
                            {
                                add_triangle(
                                    &mut vertices,
                        ...
                        if request_chunk_data
                            .get_relative_pixel(local_x, local_y - step, local_z)
                            .should_be_face(transparent)
                        {
                            add_triangle(
                                &mut vertices,
                    ...
                    if local_y + step > 15 {
                        if let Some(neighbor) = request.y_positive_chunk_neighbor.as_ref() {
                            if neighbor
                                .get_relative_pixel(local_x, -1 + step, local_z)
                                .should_be_face(transparent)
                            {
                                add_triangle(
                                    &mut vertices,
                    ...
                    if local_z == 0 {
                        if let Some(neighbor) = request.z_negative_chunk_neighbor.as_ref() {
                            if neighbor
                                .get_relative_pixel(local_x, local_y, 16 - step)
                                .should_be_face(transparent)
                            {
                                add_triangle(
                                    &mut vertices,
                        ...
                        if request_chunk_data
                            .get_relative_pixel(local_x, local_y, local_z - step)
                            .should_be_face(transparent)
                        {
                            add_triangle(
                                &mut vertices,
                    ...
                    if local_z + step > 15 {
                        if let Some(neighbor) = request.z_positive_chunk_neighbor.as_ref() {
                            if neighbor
                                .get_relative_pixel(local_x, local_y, -1 + step)
                                .should_be_face(transparent)
                            {
                                add_triangle(
                                    &mut vertices,
