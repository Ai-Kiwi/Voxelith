        ...
        for local_x in (0..16).step_by(lod as usize) {
            for local_y in (0..16).step_by(lod as usize) {
                for local_z in (0..16).step_by(lod as usize) {
                    let pixel_value = request_chunk_data.get_relative_pixel(local_x,local_y,local_z);
                    if transparent == true {
                    ...
                    if local_x == 0 {
                        ...
                        if request_chunk_data.get_relative_pixel(local_x - step, local_y, local_z).should_be_face(transparent) {
                            add_triangle(&mut vertices, TriangleSide::Left, world_pixel_x, world_pixel_y, world_pixel_z, lod, &color, &material);
                        }
                    }
                    if local_x + step > 15 {
                    ...
                    if local_y == 0 {
                        ...
                        if request_chunk_data.get_relative_pixel(local_x, local_y - step, local_z).should_be_face(transparent)  {
                            add_triangle(&mut vertices, TriangleSide::Bottom, world_pixel_x, world_pixel_y, world_pixel_z, lod, &color, &material);
                        }
                    }
                    if local_y + step > 15 {
                    ...
                    if local_z == 0 {
                        ...
                        if request_chunk_data.get_relative_pixel(local_x, local_y, local_z - step).should_be_face(transparent)  {
                            add_triangle(&mut vertices, TriangleSide::Back, world_pixel_x, world_pixel_y, world_pixel_z, lod, &color, &material);
                        }
                    }
                    if local_z + step > 15 {
