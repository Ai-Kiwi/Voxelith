...
use crate::{
    render::{
        render_frame::{
            chunks::render_opaque_chunks, entities::render_entities,
            render_world::RenderFrameObjects,
        },
        wgpu::RenderState,
    },
    utils::Vec3,
...
pub fn render_sun_shadows(
    ...
    for sun_shadow in sun_shadow_items {
        ...
        for (i, _draw_call) in render_frame_objects.terrain.iter().enumerate() {
            sun_shadow_render_pass
                .set_vertex_buffer(0, render_state.mesh_buffers[i].mesh_buffer.slice(..));
