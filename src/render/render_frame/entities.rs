...
use crate::render::{entity_meshs::{MeshId, MeshInstanceId, MeshInstanceRaw}, render_frame::render_world::RenderFrameObjects, wgpu::RenderState};

pub fn create_entities_draw_calls(render_state : &mut RenderState) -> HashMap<MeshId, Vec<MeshInstanceId>> {
...
pub fn render_entities(render_state : &RenderState, render_frame_objects : &RenderFrameObjects, render_pass : &mut wgpu::RenderPass<'_>) {
    for mesh in &render_frame_objects.entity_instances {
        ...
        render_pass.draw(vertex_info.start..(vertex_info.start+vertex_info.length), 0..(mesh.1.len() as u32));
    }
}