...
use crate::render::{entity_meshs::{MeshInstance, MeshInstanceRaw}, render_frame::render_world::RenderFrameObjects, wgpu::{LoseObjectInfo, RenderState}};
...
pub fn create_chunk_draw_calls(render_state : &mut RenderState) -> (Vec<Vec<DrawIndirectArgs>>, Vec<Vec<DrawIndirectArgs>>) {
    ...
    for (i, buffer) in render_state.mesh_buffers.iter().enumerate() {
        ...
        .par_iter()
        .filter(|mesh| mesh.index_info.transparent == false && mesh.size > 0 && mesh.buffer_number == i && mesh.lose_object_id == None)
        .map(|chunk| {
    ...
    for object in &render_state.lose_objects {
        ...
        render_state.queue.write_buffer(&render_state.static_and_lose_chunk_instance_info, (std::mem::size_of::<MeshInstanceRaw>() as u64) * current_lose_chunk_object as u64, bytemuck::bytes_of(&instance_contents_raw));
        for (i, buffer) in render_state.mesh_buffers.iter().enumerate() {
            let meshs = &buffer.meshs;

            let opaque_indirect_draw_calls: Vec<DrawIndirectArgs> = object.chunks.chunk_meshs
            .par_iter()
            .filter(|mesh| mesh.index_info.transparent == false && mesh.size > 0 && mesh.buffer_number == i && mesh.lose_object_id == Some(object.id))
            .map(|chunk| {
                let id = chunk.pointer.id;
                let mesh_info = meshs.get(&id).unwrap();
                DrawIndirectArgs {
    ...
    for (i, buffer) in render_state.mesh_buffers.iter().enumerate() {
        ...
        .par_iter()
        .filter(|mesh| mesh.index_info.transparent == true && mesh.size > 0 && mesh.buffer_number == i && mesh.lose_object_id == None)
        .map(|chunk| {
            let id = chunk.pointer.id;
            let mesh_info = meshs.get(&id).unwrap();
...
pub fn render_opaque_chunks(render_state : &RenderState, render_frame_objects : &RenderFrameObjects, render_pass : &mut wgpu::RenderPass<'_>) {
    for (i, draw_call) in render_frame_objects.terrain.iter().enumerate() {
        ...
        render_pass.set_vertex_buffer(1, render_state.static_and_lose_chunk_instance_info.slice(..));
