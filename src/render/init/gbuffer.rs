...
use crate::{
    render::{
        entity_meshs::MeshInstanceRaw,
        init::{
            sun_shadows::InitSunShadow, volumetric_lighting::create_volumetric_lighting_gbuffer,
        },
        wgpu::{RenderState, SunShadow},
    },
    utils::Vertex,
};
...
impl InitGbufferInfo {
    pub async fn new(
        ...
        //make views
        let base_color_gbuffer_view =
            base_color_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
        let lighting_gbuffer_view =
            lighting_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
        let normal_gbuffer_view =
            normal_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
        let material_gbuffer_view =
            material_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
        ...
        let gbuffers_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&base_color_gbuffer_view),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::Sampler(&base_color_gbuffer_sampler),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::TextureView(&lighting_gbuffer_view),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: wgpu::BindingResource::Sampler(&lighting_gbuffer_sampler),
                },
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: wgpu::BindingResource::TextureView(&normal_gbuffer_view),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
                    resource: wgpu::BindingResource::Sampler(&normal_gbuffer_sampler),
                },
                wgpu::BindGroupEntry {
                    binding: 6,
                    resource: wgpu::BindingResource::TextureView(&material_gbuffer_view),
                },
                wgpu::BindGroupEntry {
                    binding: 7,
                    resource: wgpu::BindingResource::Sampler(&material_gbuffer_sampler),
                },
                wgpu::BindGroupEntry {
                    binding: 8,
        ...
        let create_gbuffers_pipeline_layout =
            device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                ...
                bind_group_layouts: &[&camera_bind_group_layout, &sun_shadow.bind_group_layout],
                push_constant_ranges: &[],
            });

        //full render pipeline
        let gbuffer_render_pipeline =
            device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                ...
                vertex: wgpu::VertexState {
                    ...
                    buffers: &[Vertex::desc(), MeshInstanceRaw::desc()],
...
pub fn update_render_state_gbuffer(render_state: &mut RenderState) {
    ...
    //update gbuffers
    let base_color_gbuffer = create_base_color_gbuffer(
        &render_state.device,
        render_state.config.width,
        render_state.config.height,
    );
    let lighting_gbuffer = create_lighting_gbuffer(
        &render_state.device,
        render_state.config.width,
        render_state.config.height,
    );
    let normal_gbuffer = create_normal_gbuffer(
        &render_state.device,
        render_state.config.width,
        render_state.config.height,
    );
    let material_gbuffer = create_material_gbuffer(
        &render_state.device,
        render_state.config.width,
        render_state.config.height,
    );
    let volumetric_lighting_gbuffer = create_volumetric_lighting_gbuffer(
        &render_state.device,
        render_state.config.width,
        render_state.config.height,
    );
    ...
    //make views
    render_state.base_color_gbuffer_view =
        base_color_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
    render_state.lighting_gbuffer_view =
        lighting_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
    render_state.normal_gbuffer_view =
        normal_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
    render_state.material_gbuffer_view =
        material_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());
    render_state.volumetric_lighting_gbuffer_view =
        volumetric_lighting_gbuffer.create_view(&wgpu::TextureViewDescriptor::default());

    //remember to also update in init
    render_state.gbuffers_bind_group =
        render_state
            .device
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry {
                        ...
                        resource: wgpu::BindingResource::Sampler(
                            &render_state.base_color_gbuffer_sampler,
                        ),
                    },
                    wgpu::BindGroupEntry {
                        binding: 2,
                    ...
                    wgpu::BindGroupEntry {
                        binding: 3,
                        resource: wgpu::BindingResource::Sampler(
                            &render_state.lighting_gbuffer_sampler,
                        ),
                    },
                    wgpu::BindGroupEntry {
                        binding: 4,
                    ...
                    wgpu::BindGroupEntry {
                        binding: 5,
                        resource: wgpu::BindingResource::Sampler(
                            &render_state.normal_gbuffer_sampler,
                        ),
                    },
                    wgpu::BindGroupEntry {
                        binding: 6,
                    ...
                    wgpu::BindGroupEntry {
                        binding: 7,
                        resource: wgpu::BindingResource::Sampler(
                            &render_state.material_gbuffer_sampler,
                        ),
                    },
                    wgpu::BindGroupEntry {
                        binding: 8,
                        resource: wgpu::BindingResource::TextureView(&render_state.depth_view),
                    },
                    wgpu::BindGroupEntry {
                        binding: 9,
                        resource: wgpu::BindingResource::Sampler(&render_state.depth_sampler),
                    },
                ],
                label: Some("Gbuffers Bind Group"),
            });
    render_state.volumetric_lighting_bind_group =
        render_state
            .device
            .create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry {
                        binding: 1,
                        resource: wgpu::BindingResource::Sampler(
                            &render_state.volumetric_lighting_gbuffer_sampler,
                        ),
                    },
                ],
