...
use crate::{
    render::{
        ...
        init::{gbuffer::InitGbufferInfo, sun_shadows::InitSunShadow},
    },
    utils::Vertex,
};
...
pub fn create_volumetric_lighting_gbuffer(
    device: &wgpu::Device,
    screen_width: u32,
    screen_height: u32,
) -> wgpu::Texture {
...
impl InitVolumetricLightingInfo {
    pub fn new(
        ...
        gbuffer_data: &InitGbufferInfo,
        camera_bind_group_layout: &wgpu::BindGroupLayout,
        sun_shadow: &InitSunShadow,
    ) -> InitVolumetricLightingInfo {
        let volumetric_lighting_shader =
            device.create_shader_module(wgpu::ShaderModuleDescriptor {
                label: Some("Volumetric Lighting Shader"),
                source: wgpu::ShaderSource::Wgsl(
                    include_str!("../shaders/volumetric_lighting_shader.wgsl").into(),
                ),
            });

        //create texture infos
        let volumetric_lighting_gbuffer =
            create_volumetric_lighting_gbuffer(&device, size.width, size.height);

        //make views
        let volumetric_lighting_gbuffer_view =
        ...
        //bind group layout
        let volumetric_lighting_bind_group_layout =
        ...
        let volumetric_lighting_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&volumetric_lighting_gbuffer_view),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::Sampler(&volumetric_lighting_gbuffer_sampler),
                },
            ],
        ...
        let volumetric_lighting_render_pipeline =
