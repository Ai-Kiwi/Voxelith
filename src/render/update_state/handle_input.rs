...
pub fn handle_user_input(render_state : &mut RenderState) {
    ...
    if render_state.keys_pressed.contains_key(&winit::keyboard::KeyCode::KeyQ) {
        render_state.render_channels.input_event_tx.send(InputEvent::PlaceClick(Vec3 { 
            x: render_state.camera.position.x, 
            y: render_state.camera.position.y, 
            z: render_state.camera.position.z 
        },front)).unwrap();
    ...
    render_state.camera.pitch = if render_state.camera.pitch > 1.5 { 1.5 } else { render_state.camera.pitch };
    render_state.camera.pitch = if render_state.camera.pitch < -1.5 { -1.5 } else { render_state.camera.pitch };

    render_state.camera_uniform.update_view_proj_prespec(&mut render_state.camera, render_state.config.width, render_state.config.height);
