...
use crate::{
    ...
    render::{
        mesh::{self, create_gpu_mesh},
        wgpu::{ChunkInfo, ChunkListInfo, LoseObjectInfo, RenderState},
    },
    utils::{Mesh, Vec3},
};
...
fn get_chunk_list(
    ...
    lose_object_id: Option<LoseTerrainId>,
) -> &mut ChunkListInfo {
    let chunk_list: &mut ChunkListInfo = if let Some(some_lose_object_id) = lose_object_id {
        let mut lose_object: Option<usize> = render_state
            .lose_objects_loc
            .get(&some_lose_object_id)
            .copied();
        if lose_object.is_none() {
            ...
            render_state
                .lose_objects_loc
                .insert(some_lose_object_id, new_object_index);

            lose_object = Some(new_object_index);
        }
        &mut render_state
            .lose_objects
            .get_mut(lose_object.unwrap())
            .unwrap()
            .chunks
    } else {
...
pub fn update_render_chunk_mesh(
    render_state: &mut RenderState,
    ...
    let key = crate::render::wgpu::ChunkIndexInfo {
        x: chunk_pos.0,
        y: chunk_pos.1,
        z: chunk_pos.2,
        ...
        && mesh_data.vertices.len() > 0
    {
        ...
        match old_chunk_loc {
            ...
            None => {
                chunk_list
                    .chunk_meshs_loc
                    .insert(chunk_info.index_info, chunk_list.chunk_meshs.len());
            ...
            Some(old_loc) => {
                ...
                match old_replaced {
                    Some(replaced) => {
                        chunk_list
                            .chunk_meshs_loc
                            .insert(replaced.index_info, old_loc);
                    }
                    None => (),
                }
            }
...
pub fn update_chunk_meshs(render_state: &mut RenderState) {
    loop {
        ...
        match mesh_update {
            Ok(mesh_update) => {
                update_render_chunk_mesh(
                    render_state,
                    mesh_update.chunk_pos,
                    mesh_update.mesh.as_ref(),
                    mesh_update.transparent,
                    mesh_update.lose_object_id,
                );
            }
            Err(_) => break,
        }
