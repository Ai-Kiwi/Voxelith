...
pub fn update_entities(render_state: &mut RenderState) {
    loop {
        ...
        match entity_render_update {
            Ok(entity_update) => {
                let mut opt_entity_loc: Option<usize> =
                    render_state.entities_loc.get(&entity_update.id).copied();
                if opt_entity_loc.is_none() {
                    ...
                    render_state.queue.write_buffer(
                        ...
                        new_instance.0 * (size_of::<MeshInstanceRaw>() as u64),
                        bytemuck::bytes_of(&new_entity_instance.to_raw()),
                    );

                    instances_buffer
                        .mesh_instances
                        .insert(new_instance, new_entity_instance);
                ...
                let entity = render_state
                    .entities
                    .get_mut(opt_entity_loc.unwrap())
                    .unwrap();
                ...
                let instance_buffer = render_state
                    .mesh_instances
                    .get_mut(&entity.render_mesh_id)
                    .unwrap();
                let instance = instance_buffer
                    .mesh_instances
                    .get_mut(&entity.instance_id)
                    .unwrap();
                instance.position =
                    (entity.position.x, entity.position.y, entity.position.z).into();
                render_state.queue.write_buffer(
                    &instance_buffer.instances_buffer,
                    &entity.instance_id.0 * (size_of::<MeshInstanceRaw>() as u64),
