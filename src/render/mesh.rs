...
use crate::{
    render::{MAP_VRAM_SIZE, wgpu::RenderState},
    utils::{Mesh, Vertex},
};
...
pub fn create_gpu_mesh(render_state: &mut RenderState, mesh: &Mesh) -> Arc<GpuMeshReference> {
    ...
    if mesh_buffer_space_bytes == 0 {
        return Arc::new(GpuMeshReference {
            id: 0,
            buffer_number: 0,
        });
    }
    ...
    render_state.mesh_buffers[buffer_using]
        .free_mesh_buffer_ranges
        .retain(|x| x.byte_len != 0);

    match offset_bytes {
        Some(offset_bytes) => {
            render_state.queue.write_buffer(
                &render_state.mesh_buffers[buffer_using].mesh_buffer,
                offset_bytes.into(),
                bytemuck::cast_slice(&mesh.vertices),
            );

            let alive_pointer = Arc::new(GpuMeshReference {
                id: render_state.mesh_id_upto,
                buffer_number: buffer_using,
            });
            render_state.mesh_buffers[buffer_using].meshs.insert(
                render_state.mesh_id_upto,
                MeshBufferReference {
...
pub fn mesh_buffer_cleanup(render_state: &mut RenderState, buffer_number: usize) {
    ...
    for mut mesh in mesh_list {
        ...
        if next_mesh_pos == mesh.value().byte_vertex_position
            || mesh.value().byte_vertex_position - next_mesh_pos > MIN_FREE_SPACE_SIZE
        {
        ...
        'space_test: for free_space in &mut free_spaces {
            ...
            mesh.value_mut().vertex_position =
                free_space.byte_start / mem::size_of::<Vertex>() as u32;
        ...
        if chunk_cleanup_started.elapsed().as_secs_f32()
            > 0.03 / ((real_free_space as f32 / MAP_VRAM_SIZE as f32) * 10.0)
        {
            break;
        }
    }
