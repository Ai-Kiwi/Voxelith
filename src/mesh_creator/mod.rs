use crate::{
    render::{self, camera::PerspectiveCamera, wgpu::RenderState},
    utils::{Color, Material, Vec3, raycast_test, voxel_raycast_test},
};
...
pub fn tick_mesh_creator(
    render_state: &mut RenderState,
    mesh_creator: &mut MeshCreator,
    page_open: bool,
) {
    if page_open == false {
        return;
    ...
    mesh_creator.camera.pitch = if mesh_creator.camera.pitch > 1.5 {
        1.5
    } else {
        mesh_creator.camera.pitch
    };
    mesh_creator.camera.pitch = if mesh_creator.camera.pitch < -1.5 {
        -1.5
    } else {
    ...
    let camera_position =
        Vec3::new(0.0, 0.0, 0.0) - (camera_front_norm * mesh_creator.camera_distance);
    ...
    if render_state
        ...
        || render_state
            .keys_pressed
            .contains_key(&winit::keyboard::KeyCode::KeyR)
    {
        ...
        let normalized_mouse_x =
            (render_state.mouse_position.x - (screen_width / 2.0)) / (screen_width / 2.0);
        let normalized_mouse_y =
            -1.0 * (render_state.mouse_position.y - (screen_height / 2.0)) / (screen_height / 2.0);
        ...
        for ray in voxel_raycast_test(mesh_creator.camera.position, ray_direction) {
            ...
            if mesh_creator
                .mesh_voxels
                .get(&(ray.x as i32, ray.y as i32, ray.z as i32))
                .is_some()
            {
                if render_state
                    .keys_pressed
                    .contains_key(&winit::keyboard::KeyCode::KeyR)
                {
                    mesh_creator
                        .mesh_voxels
                        .remove(&(ray.x as i32, ray.y as i32, ray.z as i32));
                    mesh_creator.update_due = true;
                }
                break;
            }
            mesh_creator.mesh_voxels.insert(
                (ray.x as i32, ray.y as i32, ray.z as i32),
                VoxelData {
                    color: Color::new(
                        mesh_creator.selected_color[0],
                        mesh_creator.selected_color[1],
                        mesh_creator.selected_color[2],
                        255,
                    ),
                    material: Material {
                        reflectiveness: 0,
            ...
            .contains_key(&winit::keyboard::KeyCode::KeyE)
        {
            mesh_creator.mesh_voxels.insert(
                (
                    last_ray_postion.x as i32,
                    last_ray_postion.y as i32,
                    last_ray_postion.z as i32,
                ),
                VoxelData {
                    color: Color::new(
                        mesh_creator.selected_color[0],
...
pub fn render_mesh_creator(
    ...
    for sun_shadow in sun_shadow_items {
        sun_shadow.camera.target = Point3::new(
            render_state.camera_uniform.position[0],
            render_state.camera_uniform.position[1],
            render_state.camera_uniform.position[2],
        );
        sun_shadow.camera.position = Vec3::new(
            50.0 + render_state.camera_uniform.position[0],
            500.0 + render_state.camera_uniform.position[1],
            150.0 + render_state.camera_uniform.position[2],
        );
        sun_shadow
            .camera_uniform
            .update_view_proj_ortho(&mut sun_shadow.camera);
        render_state.queue.write_buffer(
            &sun_shadow.camera_buffer,
            0,
            bytemuck::cast_slice(&[sun_shadow.camera_uniform]),
        );
        ...
        if mesh_creator.mesh_buffer_size > 0 {
            if let Some(mesh_buffer) = &mesh_creator.mesh_buffer {
                ...
                sun_shadow_render_pass.set_vertex_buffer(
                    1,
    ...
    if mesh_creator.mesh_buffer_size > 0 {
        if let Some(mesh_buffer) = &mesh_creator.mesh_buffer {
            ...
            gbuffer_render_pass.set_vertex_buffer(
                1,
                render_state.static_and_lose_chunk_instance_info.slice(..),
            );
