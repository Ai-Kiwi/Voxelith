...
use crate::{mesh_creation::create_triangles::{TriangleSide, add_triangle}, mesh_creator::MeshCreator, render::wgpu::RenderState, utils::{Color, Material, Vertex}};
...
impl MeshCreator {
    pub fn vertices_from_voxel_color_hashmap(voxel_data : &HashMap<(i32,i32,i32),Color>) -> Vec<Vertex> {
        ...
        for voxel in voxel_data {
            ...
            let material = Material::new(0, 0, 0);

            if voxel_data.get(&(voxel_x - 1, voxel_y, voxel_z)).is_none() {
                add_triangle(&mut vertices, TriangleSide::Left, voxel_x, voxel_y, voxel_z, 1, voxel.1,&material);
            }
            
            if voxel_data.get(&(voxel_x + 1, voxel_y, voxel_z)).is_none() {
                add_triangle(&mut vertices, TriangleSide::Right, voxel_x, voxel_y, voxel_z, 1, voxel.1,&material);
            }

            if voxel_data.get(&(voxel_x, voxel_y - 1, voxel_z)).is_none() {
                add_triangle(&mut vertices, TriangleSide::Bottom, voxel_x, voxel_y, voxel_z, 1, voxel.1,&material);
            }

            if voxel_data.get(&(voxel_x, voxel_y + 1, voxel_z)).is_none() {
                add_triangle(&mut vertices, TriangleSide::Top, voxel_x, voxel_y, voxel_z, 1, voxel.1,&material);
            }
            
            if voxel_data.get(&(voxel_x, voxel_y, voxel_z - 1)).is_none() {
                add_triangle(&mut vertices, TriangleSide::Back, voxel_x, voxel_y, voxel_z, 1, voxel.1,&material);
            }

            if voxel_data.get(&(voxel_x, voxel_y, voxel_z + 1)).is_none() {
                add_triangle(&mut vertices, TriangleSide::Front, voxel_x, voxel_y, voxel_z, 1, voxel.1,&material);
            }
        }
        return vertices
    }
